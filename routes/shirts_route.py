...
shirts_bp = Blueprint('shirts_bp', __name__)


@shirts_bp.route('/shirts', methods=['POST'])
def createShirt():
    try:
        ...
        # Generar el id_producto automáticamente (ejemplo: PLY001, PLY002, etc.)
        ultimo_producto = Producto.query.filter(
            Producto.id_producto.like('PLY%')
        ).order_by(Producto.id_producto.desc()).first()
        
        if ultimo_producto:
            # Extraer el número del último id y incrementar
            ultimo_numero = int(ultimo_producto.id_producto.replace('PLY', ''))
            nuevo_numero = ultimo_numero + 1
        else:
            nuevo_numero = 1
        
        nuevo_id_producto = f'PLY{nuevo_numero:03d}'  # Formato: PLY001, PLY002...
        ...
        return jsonify({
            "success": False,
            "message": f"Error al crear la playera: {str(e)}"
        }), 500
